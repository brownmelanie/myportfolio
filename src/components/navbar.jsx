...
import navlogoBlack from "../assets/navbar-logo-black.svg";
...
const Navbar = () => {
  ...
  return (
    <nav className="fixed bg-background-black/55 backdrop-blur-xs top-0 left-0 w-full z-50">
      <div className={`flex items-center justify-between w-[90vw] mx-auto border-b border-solid py-1 ${isSection2 ? "border-black" : "border-white"}`}>
        <div className="z-50">
          <a href="/">
            <img
              src={isSection2 ? navlogoBlack : navlogo}
          ...
          <ul className="flex items-center space-x-8">
            {menuItems.map((item) => (
              <li key={item.title}>
                <a
                  href={item.href}
                  className={`font-primary font-light tracking-[-0.08em] text-base hover:text-[#FF4900] transition-colors duration-300 ${
                    isSection2 ? "text-black" : "text-white"
                  }`}
                >
                  {item.title}
                </a>
